...
from ..plugins.generate_ref_roi_feats import generate_ref_roi_feats, generate_ref_roi_feats_folk
...
class BHRL(TwoStageDetector):
    def __init__(self,
        ...
        self.stddev = nn.Parameter(torch.tensor(0.001))
    ...
    def extract_feat_fork1(self, img, stddev):
        img_feat = img[0]
        rf_feat = img[1]
        rf_bbox = img[2]
        img_feat = self.backbone(img_feat)
        ...
        if self.with_neck:
            ...
            rf_feat =  self.neck(rf_feat)
        img_feat_metric = self.matching(img_feat, rf_feat)
        ref_roi_feats = generate_ref_roi_feats_folk(rf_feat, rf_bbox, stddev)
        return tuple(img_feat_metric), tuple(img_feat), ref_roi_feats
    ...
    def forward_train(self,
        ...
        roi_losses = self.roi_head.forward_train(x, img_feat, ref_roi_feats, 
                                                 ...
                                                 **kwargs)
        roi_losses_folk = self.roi_head.forward_train(x, img_feat, ref_roi_feats_folk, 
                                                 img_metas, proposal_list,
                                                 gt_bboxes, gt_labels,
                                                 gt_bboxes_ignore, gt_masks,
                                                 **kwargs)
        """
        roi_losses_folk2 = self.roi_head.forward_train(x, img_feat, ref_roi_feats_folk, 
                                                 img_metas, proposal_list,
                                                 gt_bboxes, gt_labels,
                                                 gt_bboxes_ignore, gt_masks,
                                                 **kwargs)
        roi_losses_folk3 = self.roi_head.forward_train(x, img_feat, ref_roi_feats_folk, 
                                                 img_metas, proposal_list,
                                                 gt_bboxes, gt_labels,
                                                 gt_bboxes_ignore, gt_masks,
                                                 **kwargs)
        ...
        for key in roi_losses.keys():
            total_loss[key] = (roi_losses_folk[key] + roi_losses[key])/2 #+ roi_losses_folk2[key] + roi_losses_folk3[key])/4
            #print(key)
        #self.optimizer.step()
