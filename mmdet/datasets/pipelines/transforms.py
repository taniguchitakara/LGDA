...
class ReferenceTransform(object):
    def __init__(self, img_scale, mean, std, to_rgb=True, with_mask=False, keep_ratio=False):
        self.img_scale = img_scale
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        self.to_rgb = to_rgb
    ...
    def __call__(self, results):
        ...
        results['rf_img_shape'] = rf_img.shape
        results['rf_pad_shape'] = rf_img.shape
        results['rf_scale_factor'] = scale_factor
        results['rf_keep_ratio'] = self.keep_ratio
    
        if self.with_mask:
            if self.keep_ratio:
                mask = mmcv.imrescale(results['rf_mask'],
                                      scale_factor,
                                      interpolation='nearest')
            else:
                mask_size = (rf_img.shape[1], rf_img.shape[0])
                mask = mmcv.imresize(results['rf_mask'], mask_size, interpolation='nearest')

            mask = mmcv.impad(mask, self.img_scale)
            results['rf_mask'] = np.ascontiguousarray(mask)
        return results
